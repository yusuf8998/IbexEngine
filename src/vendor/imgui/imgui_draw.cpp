...
void ImFontAtlas::CalcCustomRectUV(const CustomRect* RelativeRect_, ImVec2* out_uv_min, ImVec2* out_uv_max)
{
    IM_ASSERT(TexWidth > 0 && TexHeight > 0);   // Font atlas needs to be built before we can calculate UV coordinates
    IM_ASSERT(RelativeRect_->IsPacked());                // Make sure the rectangle has been packed
    *out_uv_min = ImVec2((float)RelativeRect_->X * TexUvScale.x, (float)RelativeRect_->Y * TexUvScale.y);
    *out_uv_max = ImVec2((float)(RelativeRect_->X + RelativeRect_->Width) * TexUvScale.x, (float)(RelativeRect_->Y + RelativeRect_->Height) * TexUvScale.y);
}
...
void ImGui::RenderRectFilledRangeH(ImDrawList* draw_list, const ImRect& RelativeRect_, ImU32 col, float x_start_norm, float x_end_norm, float rounding)
{
    ...
    ImVec2 p0 = ImVec2(ImLerp(RelativeRect_.Min.x, RelativeRect_.Max.x, x_start_norm), RelativeRect_.Min.y);
    ImVec2 p1 = ImVec2(ImLerp(RelativeRect_.Min.x, RelativeRect_.Max.x, x_end_norm), RelativeRect_.Max.y);
    if (rounding == 0.0f)
    {
    ...
    rounding = ImClamp(ImMin((RelativeRect_.Max.x - RelativeRect_.Min.x) * 0.5f, (RelativeRect_.Max.y - RelativeRect_.Min.y) * 0.5f) - 1.0f, 0.0f, rounding);
    const float inv_rounding = 1.0f / rounding;
    const float arc0_b = ImAcos01(1.0f - (p0.x - RelativeRect_.Min.x) * inv_rounding);
    const float arc0_e = ImAcos01(1.0f - (p1.x - RelativeRect_.Min.x) * inv_rounding);
    const float x0 = ImMax(p0.x, RelativeRect_.Min.x + rounding);
    if (arc0_b == arc0_e)
    {
    ...
    if (p1.x > RelativeRect_.Min.x + rounding)
    {
        const float arc1_b = ImAcos01(1.0f - (RelativeRect_.Max.x - p1.x) * inv_rounding);
        const float arc1_e = ImAcos01(1.0f - (RelativeRect_.Max.x - p0.x) * inv_rounding);
        const float x1 = ImMin(p1.x, RelativeRect_.Max.x - rounding);
        if (arc1_b == arc1_e)
        {
