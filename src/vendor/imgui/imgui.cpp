...
{
    ItemSize(bb.GetScale(), text_offset_y);
}
...
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->DC.LastItemRect.GetScale();
}
...
{
    ImRect RelativeRect_ = window->Rect();
    if (thickness == 0.0f) RelativeRect_.Max -= ImVec2(1,1);
    if (border_n == 0) return ImRect(RelativeRect_.Min.x + perp_padding, RelativeRect_.Min.y,                RelativeRect_.Max.x - perp_padding, RelativeRect_.Min.y + thickness);
    if (border_n == 1) return ImRect(RelativeRect_.Max.x - thickness,    RelativeRect_.Min.y + perp_padding, RelativeRect_.Max.x,                RelativeRect_.Max.y - perp_padding);
    if (border_n == 2) return ImRect(RelativeRect_.Min.x + perp_padding, RelativeRect_.Max.y - thickness,    RelativeRect_.Max.x - perp_padding, RelativeRect_.Max.y);
    if (border_n == 3) return ImRect(RelativeRect_.Min.x,                RelativeRect_.Min.y + perp_padding, RelativeRect_.Min.x + thickness,    RelativeRect_.Max.y - perp_padding);
    IM_ASSERT(0);
    return ImRect();
}
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        else if (held)
        {
            ...
            ImVec2 corner_target = g.IO.MousePos - g.ActiveIdClickOffset + resize_rect.GetScale() * grip.CornerPos; // Corner of the window corresponding to our corner grip
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->Size = window->Collapsed && !(flags & ImGuiWindowFlags_ChildWindow) ? window->TitleBarRect().GetScale() : window->SizeFull;
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
    ...
    bool text_value_changed = InputTextEx(label, buf, IM_ARRAYSIZE(buf), aabb.GetScale(), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_AutoSelectAll);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (is_multiline)
    {
        ...
        if (!BeginChildFrame(id, frame_bb.GetScale()))
        {
            EndChildFrame();
            EndGroup();
            return false;
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect RelativeRect_(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos +ImVec2(rect_size.x, bg_offy_dn));
                    RelativeRect_.ClipWith(clip_rect);
                    if (RelativeRect_.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(RelativeRect_.Min, RelativeRect_.Max, bg_color);
                }
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(id, frame_bb.GetScale());
    return true;
}
...
void ImGui::EndGroup()
{
    ...
    if (group_data.AdvanceCursor)
    {
        ...
        ItemSize(group_bb.GetScale(), group_data.BackupCurrentLineTextBaseOffset);
        ItemAdd(group_bb, 0);
    }
